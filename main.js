...
const appendMessage = (key, messagesData) => {
    ...
    try {
        ...
        <div class="message ${type}" id="${key}">
            ...
            <p class="country">Ulke ${countryName} ${countryEmoji}</p>
            ...
            usersArr.map((user) => {
                ...
                <div class="onlineUser">
                   <div class="details">
                       ...
                       <p class="lastSeen"><b>Son Gorulme: </b><span>${lastSeen === 2000000000000 ? "Online" : moment(lastSeen).format("Do MMMM YYYY, h:mm a")}</span></p>
                   </div>
...
const typingListener = () => {
    ...
    typingStatus.on("value", (snapshot) => {
        ...
        if (isTyping) {
            $(".typingStatus p").text(`${username} yazıyor...`);
...
const toogleScroll = () => {
    if (window.autoScroll) {
        $("button.scroll").text("Otomatik Kaydırmayı Aç");
        window.autoScroll = false;
        return;
    }

    window.autoScroll = true;
    $("button.scroll").text("Otomatik Kaydırmayı Kapat");
};
